...
class ProcessContent:
    ...
    def conceptAPI(self):
        global tokens, global_weight, global_errors, input_sentence, red_flagged_words, red_flag_sentence_report, avg_weight_per_line_list
        ...
        try:
            ...
            if len(ProcessContent.list_of_weights) > 0:
                ...
                avg_weight_per_line = (sum(list_of_weight)/len(list_of_weight))
                avg_weight_per_line_list.append(avg_weight_per_line)
                print("Average of weights per line: ", avg_weight_per_line)
                print('\n')
            ...
            ProcessContent.bow = []
            ProcessContent.list_of_weights = []
            print(avg_weight_per_line_list)
            print("Length of avg_list:", len(avg_weight_per_line_list))
...
t = ProcessContent()
print("Average weight per line list: ", avg_weight_per_line_list)

# t.run('Pen will be delivered to shoot the president')
# t.run('Camera will be delivered to shoot the president')
# t.run('Profeser Mohit teaching course in a Ashoka university classroom')
# t.run('Chair teaching course in a university classroom')
# t.run('Professor farming course in a university classroom')
# t.run('Professor teaching bottle in a university classroom')
# t.run('Professor teaching course in a hospital classroom')
# t.run('Professor teaching course in a university airport')
# t.run('Professor teaching course in a university airport.')
